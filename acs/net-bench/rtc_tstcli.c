...
int send_cmd(int sockfd, char *cmd)
{
  ...
  status = net_send(sockfd, (char *) &cmd_msg, sizeof cmd_msg, BLOCKING);
  if (status <= 0)   (void) fprintf(stderr, "tstcli: net_send() error: %s\n", NET_ERRSTR(status));
  else               (void) printf("Command sent...\n");
...
int process_rsp(int sockfd)
{
  ...
  if (status < 0) {
    ...
    (void) fprintf(stderr, "tstcli: Connection closed by foreign host...\n");
  }
  else {
